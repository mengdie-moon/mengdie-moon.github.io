const quotes = [
    ...
    "你的身体是你心像的投射，你的心里越阴郁，夜晚也就越黑暗。——《老人与海》"
];
...
function displayQuote() {
    ...
    if (storedQuote && storedQuote.date === new Date().toISOString().slice(0, 10)) {
        // 如果已经存储了当天的名言，并且日期匹配，则显示存储的名言
        quoteDisplay.textContent = storedQuote.quote;
    } else {
        // 获取当天的名言并存储
        const newQuote = getQuoteForDay();
        localStorage.setItem('dailyQuote', JSON.stringify({ date: new Date().toISOString().slice(0, 10), quote: newQuote }));
...
function handleVisibilityChange() {
    if (document.hidden) {
        document.title = "呜呜呜~你舍得抛弃我吗இ௰இ";
        // 清除之前的定时器
        clearTimeout(timer);
    } else {
        document.title = "嘻嘻🤭我就赌你会回来( •̀ ω •́ )✧";
...
document.getElementById('sougo-icon').addEventListener('click', function() {
    document.getElementById('expandable-input').blur();
    toggleIcon('sougo');
    updatePlaceholder('Sogou Search...');
});

document.getElementById('csdn-icon').addEventListener('click', function() {
    document.getElementById('expandable-input').blur();
    toggleIcon('csdn');
    updatePlaceholder('CSDN Search...');
});

document.getElementById('bing-icon').addEventListener('click', function() {
    document.getElementById('expandable-input').blur();
    toggleIcon('bing');
    updatePlaceholder('Bing Search...');
});

document.getElementById('baidu-icon').addEventListener('click', function() {
    document.getElementById('expandable-input').blur();
    toggleIcon('baidu');
    updatePlaceholder('Baidu Search...');
});

function toggleIcon(engine) {
    document.querySelectorAll('.search-icon').forEach(function(icon) {
        icon.classList.remove('active');
        icon.classList.add('blank');
    });
    document.getElementById(engine + '-icon').classList.add('active');
    document.getElementById(engine + '-icon').classList.remove('blank');
}

function updatePlaceholder(text) {
    document.getElementById('expandable-input').setAttribute('placeholder', text);
}
...
function search() {
    var input = document.getElementById('expandable-input').value;
    var currentEngine = document.querySelector('.search-icon.active').id.replace('-icon', '');
    var searchUrl = '';
    switch (currentEngine) {
        case 'csdn':
            searchUrl = 'https://so.csdn.net/so/search?spm=1000.2115.3001.4498&q=' + encodeURIComponent(input);
            break;
        case 'sougo':
...
function createButton() {
    ...
    button.setAttribute('id', 'scroll-to-top'); // 设置唯一ID
    button.innerHTML = "回到顶部";
    
    // 直接在 JavaScript 中添加样式
  ...
  function handleScroll() {
    var button = document.getElementById('scroll-to-top'); // 使用ID选择按钮
  ...
  //里程碑
var button = document.createElement('button');
button.textContent = '里程碑Go';
button.id = 'myButton';

// 添加点击事件监听器
button.addEventListener('click', function() {
  window.location.href = '/About-LCB.html'; // 跳转到指定的网页
});
button.style.position = "fixed";
...
// // 尝试从localStorage获取保存的运行时间（秒）
// let totalSeconds = parseInt(localStorage.getItem('totalSeconds'), 10) || 0;

// // 更新显示的运行时间
// function updateUptime() {
//     // 计算年、月、天、小时、分钟和秒
//     const years = Math.floor(totalSeconds / (365 * 24 * 60 * 60));
//     const months = Math.floor((totalSeconds % (365 * 24 * 60 * 60)) / (30 * 24 * 60 * 60)); // 假设每月平均30天
//     const days = Math.floor((totalSeconds % (30 * 24 * 60 * 60)) / (24 * 60 * 60)) + 13; // 加上初始的13天
//     const hours = Math.floor((totalSeconds % (24 * 60 * 60)) / 3600) + 11;
//     const minutes = Math.floor((totalSeconds % 3600) / 60);
//     const seconds = totalSeconds % 60;

//     // 更新显示的运行时间
//     uptimeElement.textContent = `${years}年${months}月${days}天${hours}小时${minutes}分${seconds}秒`;
...
// }
// 设置开始时间
const startTime = new Date('2024-08-12T09:48:45Z').getTime();
...
function updateUptime() {
    ...
    // 将运行时间转换为年、月、日、小时、分钟和秒
    const years = Math.floor(elapsedTime / (1000 * 60 * 60 * 24 * 365.25));
    const months = Math.floor((elapsedTime % (1000 * 60 * 60 * 24 * 365.25)) / (1000 * 60 * 60 * 24 * 30.4375)); // 月平均30.4375天
    const days = Math.floor((elapsedTime % (1000 * 60 * 60 * 24 * 30.4375)) / (1000 * 60 * 60 * 24));
    const hours = Math.floor((elapsedTime % (1000 * 60 * 60 * 24)) / (1000 * 60 * 60));
    const minutes = Math.floor((elapsedTime % (1000 * 60 * 60)) / (1000 * 60));
    const seconds = Math.floor((elapsedTime % (1000 * 60)) / 1000);

    // 更新显示的运行时间
    document.getElementById('uptime').textContent = `${years}年 ${months}月 ${days}日 ${hours}小时 ${minutes}分钟 ${seconds}秒`;
}
...
function setLocalStorage(name, value) {
    if (!caution || (name + "=" + value).length <= 4000) {
        localStorage.setItem(name, value);
    } else {
        if (confirm("Storage exceeds 4KB and will be cut!")) {
            localStorage.setItem(name, value);
        }
    }
}
...
var visits = getLocalStorage("counter");
...
setLocalStorage("counter", visits);

// 2024-9-14 新增功能：访问量显示
let db;
const request = window.indexedDB.open("visitCounterDB", 1);
...
function readCount() {
    ...
    const request = store.get(1); // 假设我们总是获取键为1的记录

    request.onsuccess = function(event) {
        const count = event.target.result ? event.target.result.value : 0;
        updateCount(count + 1);
    };
}
...
function updateCount(newValue) {
    ...
    const store = transaction.objectStore("visits");
    const request = store.put({ id: 1, value: newValue });

    request.onsuccess = function() {
        // 获取div.sss p元素，并更新其内容
        const pElement = document.querySelector('div.page-top p');
        if (pElement) {
            pElement.textContent = "吸吸🤭这是你第"+newValue+"次看我 加油哦逐梦者";
        } else {
    ...
    request.onerror = function(event) {
        console.error("Error updating count: " + event.target.errorCode);
    };
}
//2024-9-14 因为不可避免原因 决定塞上一坨史
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    button.textContent = '公告栏';
    button.style.position = "fixed";
    button.style.bottom = "14%";
    button.style.right = "30px";
    button.style.padding = "10px 20px";
    button.style.border = "none";
    button.style.backgroundColor = "#3498db";
    ...
    button.addEventListener('click', function() {
        Swal.fire({
            title: '欢迎各位来到这里 Hello!',
            html: `
                <p>对<span>检查官</span>的话: 本站已经禁用了检查 还能打开控制台的有<span>惊喜！</span></p>
                <hr>
                <p>例外本站啥用没有呜呜 没有技术文章~但是<span style="color: rgb(207, 190, 166);">正在努力更新了</span>例外这个弹窗有一个<span>bug,But我不想修 只需要不重复刷新首页就可以了！</span></p>
                <p>本站源码尽在<span style="color: rgb(207, 190, 166);">'源码下载'</span>一栏里 还有各种我以前写过的源码！</p>
                <p>需要技术文章请切换到<span>csdnSearch</span>后续会支持<span>githubSearch</span>,<span>博客圆Search</span></p>
                <p>啊吧啊吧 目前黑名单：<span style="color: rgb(207, 190, 166);">F12, 右键, 打开控制台 嘻嘻✌</span></p>
                <p>❗QQ: 597569159--群 希望有大佬送上门指点我[doge]❗</p>
                <p>最近更新时间: 2024-9-15 中秋快乐！！</p>
            `,
    ...
    function inputHTML() {
        Swal.fire({
            ...
            html: `
                ...
                <p style="font-size: 14px; ">待更新...(CSS部分失效, 上传图片等) 链接请用网址 部分代码因布局失效 部分代码会影响网站布局(可修但是我好懒)</p>
                <textarea id="userHTML" placeholder="请输入HTML代码..." rows="10"></textarea>
                <textarea id="userCSS" placeholder="请输入CSS代码..." rows="10"></textarea>
                <textarea id="userJS" placeholder="请输入JavaScript代码...(测试ing 只能运行小部分 使用时删除script标签)" rows="10"></textarea>
            `,
            width: 900,
            showCancelButton: false,
            confirmButtonText: '运行代码'
        }).then((result) => {
            if (result.isConfirmed) {
                const html = document.getElementById('userHTML').value;
                const css = document.getElementById('userCSS').value;
                const js = document.getElementById('userJS').value;
                displayHTMLandCSS(html, css, js);
            }
        });
    }
    
    function displayHTMLandCSS(html, css, js) {
        Swal.fire({
            toast: true,
            position: 'top-end',
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    button.addEventListener('click', function() {
        Swal.fire({
            title: '注意！！',
            // html: `
            //     <p>确定要使用吗 请勿大规模传递传播！ 我还不想收到律师函 请遵守他人电影版权</p>
            // `,
            html: `
                <p>此功能暂时关闭！！国庆日开放 与国与民同乐</p>
            `,
            showCloseButton: true,
            showCancelButton: true,
            // confirmButtonText: '我已熟知并遵守',
...
// 添加点击事件监听器
button.addEventListener('click', function() {
  window.location.href = 'advices.html';
});
button.style.position = "fixed";
button.style.bottom = "26%";
button.style.right = "30px";
button.style.padding = "10px 20px";
button.style.border = "none";
button.style.backgroundColor = "#3498db";
